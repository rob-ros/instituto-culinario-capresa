...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (carousel) {
    ...
    let indicators = carousel.querySelectorAll('.carousel-indicator');
    const indicatorsContainer = carousel.querySelector('.carousel-indicators');
    
    let currentIndex = 0;
    const totalSlides = slides.length;

    // Generar indicadores automáticamente si la cantidad no coincide con las diapositivas
    if (indicatorsContainer) {
      if (indicators.length !== totalSlides) {
        ...
        // Asegurar que el estado inicial esté sincronizado (indicadores y labels)
        updateCarousel();
      }
    }
    ...
    function goToSlide(index) {
      ...
      updateCarousel();
      // Reiniciar autoplay para asegurar duración consistente entre cambios
    ...
    // Auto-play opcional (cada 3 segundos)
  ...
  // =========================================================================
  // Botón flotante de WhatsApp: ocultar al entrar el footer
  // =========================================================================
  (function() {
    const whatsappFab = document.querySelector('.whatsapp-fab');
    const footerEl = document.querySelector('footer');
    if (!whatsappFab || !footerEl) return;
    ...
    if ('IntersectionObserver' in window) {
      ...
      // Fallback: hide when footer becomes visible in viewport
    ...
    // Modo 2 pasos para pantallas pequeñas: primer toque expande, segundo abre
    function isSmallViewport() {
      return window.matchMedia('(max-width: 768px)').matches;
    }

    function closeOnOutsideClick(e) {
      if (!whatsappFab.contains(e.target)) {
        whatsappFab.classList.remove('is-open');
        document.removeEventListener('click', closeOnOutsideClick, true);
      }
    }

    whatsappFab.addEventListener('click', function(e) {
      if (!isSmallViewport()) return; // en desktop permitir comportamiento normal

      if (!whatsappFab.classList.contains('is-open')) {
        // Primer toque: expandir y prevenir navegación
        e.preventDefault();
        whatsappFab.classList.add('is-open');
        // Cerrar al tocar fuera
        setTimeout(function(){
          document.addEventListener('click', closeOnOutsideClick, true);
        }, 0);
      } else {
        // Segundo toque: dejar navegar (no prevenir)
        // Se removerá la clase al tocar fuera o al redimensionar
      }
    });

    // Al cambiar tamaño de viewport, limpiar estado abierto
